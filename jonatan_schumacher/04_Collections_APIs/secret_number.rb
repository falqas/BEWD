...
#get layer's name and greet them
...
def play_the_game(secret_num, num_rounds)
	...
	while num_remaining_trials > 0 do
		#get their number
		arr_guesses << get_input_number("You have #{num_remaining_trials} #{num_remaining_trials > 1? "guesses" : "guess"} left. Please input a number between 1 and 10:",1,10)
		...
		if (arr_guesses.last.to_i == secret_num)
			...
			animate_message("Guess a higher number!".upcase,0.75) if num_remaining_trials > 1
		else
			puts "Number #{arr_guesses.last} is too high.\n"
			animate_message("Guess a lower number!".upcase,0.75) if num_remaining_trials > 1
		end
...
if (result.last.to_i == the_secret_number) then
	...
	puts "Congratulations. You won this round, #{username}! It took you #{result.size} #{result.size > 1? "attempts" : "attempt"}."
else
