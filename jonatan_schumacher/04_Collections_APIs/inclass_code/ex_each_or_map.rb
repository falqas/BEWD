...
blog_posts.each do |i|
  #get the arrays and add to all_tags
  all_tags << i[:tags]
  #binding.pry
end
...
##############################
## doing it all in a nested loop:
new_tags = []
blog_posts.each do |p|
  # loop through each hash and look at :tags
  p[:tags].each do |tag|
    if !new_tags.include? tag
      new_tags << tag
    end
  end
  # if new_tags does not contain tags value, then add it
end
##############################


##############################
##using maps instead of each:
tags = blog_posts.map do |item|
  item[:tags]#.capitalize
...
if new_tags != ['art', 'education', 'science', 'discovery', 'dialogues', 'history', 'meter', 'storytelling', 'morality', 'prison']
